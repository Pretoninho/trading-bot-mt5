...
L'interface s'ouvrira à : **http://localhost:8501**
...
| **Partial close** | 10-50% | 25% | % postion fermée à chaque TP |
...
| **Invalid action penalty** | -1e-4 | Float | Pénalité actions bloquées |
...
| **Hidden dim** | 128 | 64 | 512 | 64=rapide/léger, 256=balanc, 512=puissant |
...
**Q: "Module not found: bot_config"**  
A: Assurez-vous que `bot_config.py` est à la racine du projet avec `bot_config_interface.py`

**Q: Streamlit demande des dependences PyTorch**  
A: Installez torch séparément :
```bash
pip install torch torchvision torchaudio --index-url https://download.pytorch.org/whl/cpu
```

**Q: Les fichiers uploadés ne sont pas sauvegardés**  
A: Files uploadés via Streamlit ne sont que dans la session. Copiez-les manuellement dans le projet.
